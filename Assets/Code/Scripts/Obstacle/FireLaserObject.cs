...
public class FireLaserObject : MonoBehaviour
{
    ...
    public GameObject fireEffectPrefab;   // 🔥 불꽃 프리팹
    public float fireSpacing = 0.3f;      // 🔥 불꽃 간격
    private List<GameObject> activeFires = new List<GameObject>(); // 🔥 활성 불 목록
    ...
    private IEnumerator SpawnFireEffectsAlongLaser()
    {
        if (fireEffectPrefab == null) yield break;
        ...
        // 🔥 일정 간격으로 불꽃 생성
        for (float d = 0; d <= distance; d += fireSpacing)
        {
            Vector3 spawnPos = startPos + dir * d;
            GameObject fire = Instantiate(fireEffectPrefab, spawnPos, Quaternion.identity);
    ...
    private IEnumerator AnimateStartCircleAndLaser()
    {
        ...
        // 🔥 레이저가 완전히 다 뻗은 직후 불꽃 생성
        StartCoroutine(SpawnFireEffectsAlongLaser());
